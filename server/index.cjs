...
function normalizeEntryDateTime(value, fallback = null) {
  ...
  if (dateOnly) {
    const now = new Date();
    return `${raw}T${cairoTimeSuffix(now)}${timeZoneOffsetSuffix(now, 'Africa/Cairo')}`;
  }
  const serialDate = excelSerialToIso(raw);
  if (serialDate && serialDate !== raw && /^(\d{4})-(\d{2})-(\d{2})$/.test(serialDate)) {
    const now = new Date();
    return `${serialDate}T${cairoTimeSuffix(now)}${timeZoneOffsetSuffix(now, 'Africa/Cairo')}`;
  }
  return raw;
}
...
function releaseVersion(release) {
  const candidates = [
    release?.tag_name,
    release?.name,
    ...(Array.isArray(release?.assets) ? release.assets.map((asset) => asset.name) : []),
  ];
  let best = '';
  for (const candidate of candidates) {
    const version = extractVersion(candidate);
    if (/^\d+\.\d+\.\d+/.test(version) && (!best || compareVersions(version, best) > 0)) {
      best = version;
    }
  }
  return best || extractVersion(release?.tag_name || release?.name || APP_VERSION) || APP_VERSION;
}
...
function chooseBestRelease(releases, platform = '') {
  const sorted = (Array.isArray(releases) ? releases : [])
    ...
    .sort((left, right) => {
      const versionDiff = compareVersions(releaseVersion(right), releaseVersion(left));
      if (versionDiff !== 0) return versionDiff;
      return Date.parse(right.published_at || right.created_at || 0) - Date.parse(left.published_at || left.created_at || 0);
    });
  return sorted.find((release) => chooseReleaseAsset(release, platform)) || sorted[0] || null;
}
...
function normalizeInput(database, body, existing = {}) {
  ...
  const entryDate = normalizeEntryDateTime(item.entry_date, document?.entry_date || new Date().toISOString());
...
function totalsForRows(rows, document = null) {
  const totals = rows.reduce((acc, row) => {
    ...
    acc.social_insurance_amount += effectiveAmount(row, 'social_insurance_amount');
    acc.stamp_amount += effectiveAmount(row, 'stamp_amount');
    acc.works_insurance_amount += effectiveAmount(row, 'works_insurance_amount');
    acc.final_insurance_amount += effectiveAmount(row, 'final_insurance_amount');
    acc.contractor_tax_amount += effectiveAmount(row, 'contractor_tax_amount');
    acc.deductions += effectiveAmount(row, 'social_insurance_amount')
    ...
    quantity: 0, area_m2: 0, cost: 0, gross_total: 0, real_gross_total: 0, real_net_total: 0,
    vat_amount: 0, social_insurance_amount: 0, stamp_amount: 0, works_insurance_amount: 0,
    final_insurance_amount: 0, contractor_tax_amount: 0, deductions: 0, collections: 0,
    net_before_discount: 0, discount_amount: 0, net_total: 0,
  });
...
function reportUsesCompletion(type) {
  return ['invoice', 'taxInvoice', 'nonTaxInvoice', 'statement', 'taxStatement', 'nonTaxStatement', 'contractor'].includes(type);
}

function enrichCompletionRow(row, type = '') {
  const percent = numberOrZero(row.collection_amount) ? 100 : completionPercent(row.completion_ratio ?? row.completion_percent);
...
function effectiveAmount(row, key) {
  ...
  if (workKey && Object.prototype.hasOwnProperty.call(row, workKey)) return numberOrZero(row[workKey]);
  return numberOrZero(row[key]);
}

function formatCompletion(row) {
  const percent = completionPercent(row.completion_percent ?? row.completion_ratio);
...
function contractorRowsUpToCertificate(rows, requested) {
  ...
  const selectedIndex = groups.findIndex((group) => String(group.key) === String(selected));
  if (selectedIndex >= 0) {
    const allowedIds = new Set(groups.slice(0, selectedIndex + 1).flatMap((group) => group.rows.map((row) => row.id)));
...
function hasReportDimensions(rows) {
  return rows.some((row) => {
    ...
    if (row.unit_code === 'sqm') return width > 0 && height > 0;
    if (row.unit_code === 'lm') return width > 0;
    return false;
  });
}
...
function dimensionText(row, unit = 'cm') {
  const width = numberOrZero(row.width_cm);
  const height = numberOrZero(row.height_cm);
  const count = numberOrZero(row.item_count) || 1;
  if (row.unit_code === 'sqm') {
    if (!width || !height) return '';
    return unit === 'm'
      ? `${money(width / 100)} × ${money(height / 100)} م`
      : `${money(width)} × ${money(height)} سم`;
  }
  if (row.unit_code === 'lm') {
    if (!width) return '';
    const lengthText = unit === 'm' ? `${money(width / 100)} م` : `${money(width)} سم`;
    return count > 1 ? `${lengthText} × ${money(count)}` : lengthText;
  }
  return '';
}
...
function accountStatementData(database, query, type) {
  ...
  const totals = statementRows.reduce((acc, row) => {
    ...
    quantity: 0, real_gross_total: 0, real_debit: 0, gross_total: 0, vat_amount: 0, social_insurance_amount: 0, stamp_amount: 0,
  ...
  return {
    ...
    project: query.project || (statementProjects.length > 1 ? 'كل المشاريع' : (statementProjects[0] || '')),
    building_unit: query.project ? (first.building_unit || query.building_unit || '') : '',
...
function getReportRows(database, query, type) {
  const isCumulativeContractor = type === 'contractor' && normalizeText(query.certificate_no);
  const reportQuery = isCumulativeContractor ? { ...query, certificate_no: '' } : query;
  ...
  if (isCumulativeContractor) rows = contractorRowsUpToCertificate(rows, query.certificate_no);
...
function highlightPercentages(value) {
  return escapeHtml(value).replace(/(\d+(?:\.\d+)?\s*%)/g, '<span class="percent-red">$1</span>');
}
...
function taxBreakdown(rows) {
  return [
    ...
    key,
    label,
    amount: roundMoney((rows || []).reduce((sum, row) => sum + effectiveAmount(row, key), 0)),
  })).filter((item) => item.amount);
}
    ...
    <table class="report-table payment-table">
      ...
      <tbody>${rows.map((row) => `
        <tr class="payment-row">
          ...
          <td class="number-cell">${money(Math.abs(numberOrZero(row.collection_amount)))}</td>
        </tr>`).join('')}</tbody>
    </table>`;
}
...
function detailReportTableHtml(data, showDimensions, dimensionUnit) {
  ...
  if (data.type === 'contractor') {
    ...
    for (const group of groups) {
      body.push(`<tr class="group-row certificate-row"><td colspan="10">مستخلص رقم ${escapeHtml(group.key)}</td></tr>`);
      group.rows.forEach((row) => {
        body.push(`
          <tr>
            ...
            <td class="number-cell">${money(row.gross_total)}</td>
            <td class="number-cell">${money(effectiveAmount(row, 'gross_total'))}</td>
          </tr>`);
      });
    ...
    return `
      <table class="report-table contractor-detail">
        <thead><tr><th>التاريخ</th><th>البيان</th><th>الموقع</th><th>الوحدة</th><th>العدد</th><th>الكمية</th><th>نسبة العمل</th><th>الفئة</th><th>الإجمالي</th><th>بعد النسبة</th></tr></thead>
        <tbody>${body.join('')}</tbody>
      </table>`;
  }
  const generalWorksLabel = '\u0627\u0639\u0645\u0627\u0644 \u0639\u0627\u0645\u0629';
  const showCompletionColumn = ['invoice', 'taxInvoice', 'nonTaxInvoice'].includes(data.type);
  ...
  for (const row of orderedRows) {
    ...
    const totals = locationTotals.get(key) || { quantity: 0, item_count: 0, gross_total: 0, work_gross_total: 0, net_total: 0 };
    totals.quantity += numberOrZero(row.quantity);
    totals.item_count += numberOrZero(row.item_count);
    totals.gross_total += numberOrZero(row.gross_total);
    totals.work_gross_total += effectiveAmount(row, 'gross_total');
    ...
    const unit = unitTotals.get(unitKey) || { quantity: 0, item_count: 0, gross_total: 0, work_gross_total: 0, net_total: 0 };
  ...
  orderedRows.forEach((row, index) => {
    ...
    if (showLocation && key !== activeLocation) body.push(`<tr class="group-row"><td colspan="${6 + (showDimensions ? 1 : 0) + (showCompletionColumn ? 2 : 0)}">${escapeHtml(key)}</td></tr>`);
    activeLocation = key;
    activeUnit = unitKey;
    ...
    body.push(`
      <tr>
        ...
        <td class="number-cell">${displayNumber(row.rate, true)}</td>
        <td class="number-cell">${money(row.gross_total)}</td>
        ${showCompletionColumn ? `<td class="number-cell">${money(effectiveAmount(row, 'gross_total'))}</td>` : ''}
      </tr>`);
...
function totalsHtml(data) {
  ...
  if (data.is_statement || (data.statementRows || []).length) {
    ...
    const realGross = numberOrZero(totals.real_gross_total || totals.real_debit);
    const workGross = numberOrZero(totals.gross_total || totals.debit);
    const workRateBox = realGross && roundMoney(realGross) !== roundMoney(workGross)
    ...
    return `
      <div class="totals">
        ${quantitySummary}
        ${workRateBox || `<div class="box"><span>إجمالي المستندات</span><strong>${money(workGross)}</strong></div>`}
        ${taxBoxes}
        ${discountBox}
  ...
  return `
    <div class="totals">
      ${quantityBox}
      ${itemCountBox}
      ${workRateBox || (showGross ? `<div class="box"><span>الإجمالي</span><strong>${money(totals.gross_total)}</strong></div>` : '')}
      ${taxBoxes}
      ${discountBox}
      ${paymentBox}
...
function renderReportHtmlV2(data) {
  ...
  const projectValue = projectValues.length === 1 ? projectValues[0] : (data.project || 'كل المشاريع');
  const buildingValue = data.type === 'contractor' ? '' : (buildingValues.length === 1 ? buildingValues[0] : (buildingValues.length > 1 ? 'كل المشاريع' : data.building_unit || ''));
  const headerInfo = [
  ...
  const statementHtml = statementRows.length ? `
    <table class="report-table">
      <thead><tr><th>التاريخ</th><th>المستند</th><th>المشروع</th><th>التفاصيل</th><th>الكمية</th><th>نسبة العمل</th><th>ضرائب / خصم</th><th>إجمالي المستند</th><th>التحصيل</th><th>الرصيد</th></tr></thead>
      <tbody>${statementRows.map((row) => `
        <tr class="${row.is_payment ? 'payment-row' : ''}">
          ...
          <td class="number-cell">${row.is_payment ? '-' : money(row.quantity)}</td>
          ...
          <td class="number-cell">${money(row.debit)}</td>
          <td class="number-cell">${money(row.credit)}</td>
          <td class="number-cell">${money(row.balance)}</td>
        </tr>`).join('')}</tbody>
    </table>` : '';
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .info-item strong{font-weight:800;text-align:right}
    .report-table{width:max-content;min-width:100%;border-collapse:collapse;margin-top:10px;font-size:11px;page-break-inside:auto}
    .report-table thead{display:table-header-group}
    .report-table tfoot{display:table-footer-group}
    .report-table tbody,.report-table tr,.report-table th,.report-table td{break-inside:avoid!important;break-inside:avoid-page!important;page-break-inside:avoid!important}
    ...
    .nowrap,.date-cell,.id-cell,.number-cell,.dimension-cell,.unit-cell{white-space:nowrap;word-break:keep-all;overflow-wrap:normal}
    ...
    .percent-red,.work-rate-cell{color:#c00000;font-weight:700}
...
async function writeCleanXlsx(data, outputPath) {
  ...
  const showCompletion = !isStatement && ['invoice', 'taxInvoice', 'nonTaxInvoice', 'contractor'].includes(data.type);
  const columns = isStatement
    ...
    : [
      ...
      { header: 'الكمية', key: 'quantity', width: 12 },
      ...(showCompletion ? [{ header: 'نسبة العمل', key: 'completion', width: 12 }] : []),
      { header: 'الفئة', key: 'rate', width: 13 },
      { header: 'الإجمالي', key: 'gross', width: 16 },
      ...(showCompletion ? [{ header: 'بعد النسبة', key: 'work_gross', width: 16 }] : []),
    ];
  const sheet = workbook.addWorksheet('التقرير', {
    views: [{ rightToLeft: true, state: 'frozen', ySplit: 7 }],
    pageSetup: { orientation: 'portrait', fitToPage: true, fitToWidth: 1 },
  ...
  const realTotal = numberOrZero(data.totals?.real_gross_total || data.totals?.real_debit);
  const workTotal = numberOrZero(data.totals?.gross_total || data.totals?.debit);
...
function buildReportData(database, query, type) {
  if (['statement', 'taxStatement', 'nonTaxStatement'].includes(type)) return accountStatementData(database, query, type);
  ...
  const displayProject = document?.project || query.project || (projects.length > 1 ? 'كل المشاريع' : (projects[0] || first.project || ''));
  const rowDates = rows.map((row) => normalizeText(row.entry_date)).filter(Boolean).sort();
  const paymentQuery = type === 'contractor' && normalizeText(query.certificate_no) && rowDates.length
...
function reportFileName(data, extension) {
  if (data.type === 'contractor') {
    const certificateNo = data.certificate_no || data.operation_no || data.serial || Date.now();
  ...
  app.get('/api/update/latest', async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        downloadUrl: asset?.browser_download_url || release.html_url || `https://github.com/${repo}/releases`,
